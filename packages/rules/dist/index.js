 ...
 */
export function connectionTypeFitsCapabilities(from, to, type) {
    switch (type) {
        ...
        case "database-access":
            ...
            return ((hasCap(from, "state-access") && hasCap(to, "state-access")) ||
...
export function canConnect(model, fromId, toId, type, ports) {
    ...
    if (portCheck)
        return portCheck;
    const matrixMatch = SUGGEST_RULES.some((r) => r.from.includes(from) &&
        r.to.includes(to) &&
