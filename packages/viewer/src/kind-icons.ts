 ...
 */
export const kindVisuals: Record<NodeKind, KindVisual> = {
  service: {
    color: "#3b82f6",
    soft: "#eff6ff",
    icon: [
...
const edgeIcons: Record<SphereEdge["kind"], { color: string; icon: IconChild[] }> = {
  rest: {
    color: "#334155",
    icon: [
      {
...
export function edgeVisual(kind: SphereEdge["kind"]) {
...
export function renderLucideIcon(
  ...
  const body = children
    .map((c) => {
      if (c.tag === "path") return `<path d="${c.d}"/>`;
      if (c.tag === "circle") return `<circle cx="${c.cx}" cy="${c.cy}" r="${c.r}"/>`;
      if (c.tag === "ellipse")
