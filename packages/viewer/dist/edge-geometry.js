...
const HOP_RADIUS = 7;
const EPS = 0.5;
export function anchorPoint(n, side, 
 ...
 */
export function pickEdgeSides(from, to) {
...
export function fanAlongSide(index, count) {
    if (count <= 1)
        return 0.5;
...
export function orthoLaneOffset(index, count, gap = ORTHO_LANE_GAP) {
    if (count <= 1)
        return 0;
    const i = Math.max(0, Math.min(count - 1, index));
 ...
 */
export function resolveEdgeAnchors(from, to, fanIndex = 0, fanCount = 1) {
    ...
    const toAlong = toSide === "l" || toSide === "r"
        ? along
...
export function edgeControls(a, b, aSide) {
    const dx = b.x - a.x;
    const dy = b.y - a.y;
...
export function edgePath(a, b, aSide, bSide, mode = "bezier", lane = 0) {
    if (mode === "orthogonal") {
...
function dedupePoints(points) {
    ...
    for (const p of points) {
        ...
        if (!prev || !almostEq(prev.x, p.x) || !almostEq(prev.y, p.y))
            out.push(p);
    }
    return out;
}
...
export function projectOntoSide(box, p, side) {
    switch (side) {
        case "l":
 ...
 */
export function sideFacingPoint(box, p, hint) {
    const left = box.x;
    ...
    if (hint) {
        ...
        if (hinted && hinted[1] <= Math.min(...dists.map((d) => d[1])) + 1)
            return hint;
    }
    dists.sort((a, b) => a[1] - b[1]);
    return dists[0][0];
}
function sideAllowsOrthogonalStub(box, p, side) {
 ...
 */
export function resolveAnchorsFromWaypoints(from, to, waypoints, hintFrom, hintTo) {
    const w0 = waypoints[0] ?? { x: from.x + from.w, y: from.y + from.h / 2 };
    const wN = waypoints[waypoints.length - 1] ?? {
        x: to.x,
        y: to.y + to.h / 2,
    };
    const fromSide = hintFrom && sideAllowsOrthogonalStub(from, w0, hintFrom)
        ? hintFrom
        : sideFacingPoint(from, w0, hintFrom);
    const toSide = hintTo && sideAllowsOrthogonalStub(to, wN, hintTo)
        ? hintTo
 ...
 */
export function normalizeOrthogonalPolyline(points) {
    if (points.length < 2)
 ...
 */
export function clampOrthogonalRouteEnds(points, from, to, hintFrom, hintTo) {
    if (points.length < 2)
        return points.map((p) => ({ x: p.x, y: p.y }));
...
export function orthogonalWaypoints(a, b, aSide, bSide = "l", stub = ORTHO_STUB, 
...
function hvCrossing(h, v) {
    ...
    if (x <= hMinX + EPS || x >= hMaxX - EPS)
        return null;
    if (y <= vMinY + EPS || y >= vMaxY - EPS)
        return null;
    return { x, y };
}
function crossingsOnSegment(seg, others) {
    const hits = [];
 ...
 */
export function orthogonalPathWithHops(points, earlierSegments, hopRadius = HOP_RADIUS) {
...
function resolveRoutedEdge(e) {
    if (e.from && e.to && e.waypoints?.length) {
        const resolved = resolveAnchorsFromWaypoints(e.from, e.to, e.waypoints, e.aSide ? asSide(e.aSide) : undefined, e.bSide ? asSide(e.bSide) : undefined);
        return {
            id: e.id,
            a: resolved.a,
            b: resolved.b,
    ...
    if (e.from && e.to) {
        const resolved = resolveEdgeAnchors(e.from, e.to, e.fanIndex ?? 0, e.fanCount ?? 1);
        return {
            id: e.id,
            a: resolved.a,
            b: resolved.b,
 ...
 */
export function assignOrthogonalLanes(edges) {
 ...
 */
export function routeOrthogonalEdges(edges, hopRadius = HOP_RADIUS) {
    const out = new Map();
    const earlier = [];
...
export function routeOrthogonalPolylines(edges) {
    const out = new Map();
    const lanes = assignOrthogonalLanes(edges);
    for (const e of edges) {
        const r = resolveRoutedEdge(e);
...
export function pointOnCubic(p0, c1, c2, p3, t) {
    const u = 1 - t;
    const tt = t * t;
    const uu = u * u;
    const uuu = uu * u;
...
export function tangentOnCubic(p0, c1, c2, p3, t) {
    const u = 1 - t;
    return {
...
function rectsOverlap(ax, ay, aw, ah, b, pad = 4) {
    return (ax - pad < b.x + b.w &&
        ax + aw + pad > b.x &&
        ay - pad < b.y + b.h &&
        ay + ah + pad > b.y);
}
function labelHitsNodes(cx, cy, labelW, labelH, nodes) {
 ...
 */
export function placeEdgeLabel(opts) {
    ...
    if (opts.fromBox && opts.toBox) {
        const resolved = resolveEdgeAnchors(opts.fromBox, opts.toBox, opts.fanIndex ?? 0, opts.fanCount ?? 1);
        a = resolved.a;
        b = resolved.b;
        aSide = resolved.fromSide;
        bSide = resolved.toSide;
 ...
 */
export function computeLabelStagger(placements, threshold = 56) {
...
function centeredOverlap(a, b, gap) {
...
export function estimateEdgeLabelSize(label, contract) {
 ...
 */
export function resolveLabelOverlaps(placements, opts = {}) {
