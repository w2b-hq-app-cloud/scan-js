...
import { clampOrthogonalRouteEnds, computeLabelStagger, edgeControls, edgePath, orthogonalWaypoints, pickEdgeSides, placeEdgeLabel, pointOnCubic, resolveAnchorsFromWaypoints, resolveEdgeAnchors, resolveLabelOverlaps, routeOrthogonalEdges, routeOrthogonalPolylines, } from "./edge-geometry.js";
...
test("parallel orthogonal same-pair edges use distinct mid lanes", () => {
    ...
    const pts = (d) => [...d.matchAll(/[ML]\s*([\d.]+)\s+([\d.]+)/g)].map((m) => ({
        x: Number(m[1]),
        y: Number(m[2]),
    }));
    const midRunCoord = (d) => {
        ...
        for (let i = 0; i < p.length - 1; i++) {
            ...
            if (dx > 40 && dy < 0.5) {
                ...
                if (!best || len > best.len)
                    best = { axis: "y", value: p[i].y, len };
            }
            else if (dy > 40 && dx < 0.5) {
                const len = dy;
                if (!best || len > best.len)
                    best = { axis: "x", value: p[i].x, len };
            }
        }
    ...
    assert.ok(Math.abs(m0.value - m1.value) >= 16, `expected lane gap on ${m0.axis}, got ${m0.value} vs ${m1.value}`);
});
test("pickEdgeSides attaches to facing edges from travel direction", () => {
    const right = pickEdgeSides({ x: 0, y: 0, w: 100, h: 80 }, { x: 200, y: 10, w: 100, h: 80 });
    assert.deepEqual(right, { fromSide: "r", toSide: "l" });
    const below = pickEdgeSides({ x: 0, y: 0, w: 100, h: 80 }, { x: 10, y: 160, w: 100, h: 80 });
    assert.deepEqual(below, { fromSide: "b", toSide: "t" });
    const left = pickEdgeSides({ x: 200, y: 0, w: 100, h: 80 }, { x: 0, y: 10, w: 100, h: 80 });
...
test("resolveLabelOverlaps stacks overlapping chips with a gap", () => {
    ...
    for (const [p, q] of pairs) {
        ...
        assert.ok(dy >= 49 || dx >= 95, `labels still overlap: (${p.x},${p.y}) vs (${q.x},${q.y})`);
    }
});
