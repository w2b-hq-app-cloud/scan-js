...
import { edgePath, placeEdgeLabel, resolveEdgeAnchors, routeOrthogonalEdges, assignOrthogonalLanes, resolveLabelOverlaps, estimateEdgeLabelSize, } from "./edge-geometry.js";
...
function renderPortRow(ports, x, y, width, color, side) {
    return ports
        .map((p, i) => {
        const py = y + i * 14;
        const label = p.protocol ? `${p.label} (${p.protocol})` : p.label;
        if (side === "in") {
            return (`<circle cx="${x + 4}" cy="${py}" r="3" fill="none" stroke="${color}" stroke-width="1.5"/>` +
                `<text x="${x + 12}" y="${py + 3}" font-family="system-ui,sans-serif" font-size="10" fill="#0f172a">${escapeXml(label)}</text>`);
        }
        return (`<text x="${x + width - 12}" y="${py + 3}" text-anchor="end" font-family="system-ui,sans-serif" font-size="10" fill="#0f172a">${escapeXml(label)}</text>` +
            `<circle cx="${x + width - 4}" cy="${py}" r="3" fill="${color}" stroke="${color}" stroke-width="1"/>`);
    })
        .join("");
}
function renderDbCylinder(x, y, width, height, color) {
    ...
    const body = `<path d="M ${cx - rx} ${topY} L ${cx - rx} ${bottomY} A ${rx} ${ry} 0 0 0 ${cx + rx} ${bottomY} L ${cx + rx} ${topY} A ${rx} ${ry} 0 0 1 ${cx - rx} ${topY}" fill="white" stroke="${stroke}" stroke-width="1.5"/>` +
        [0.28, 0.52, 0.76]
            .map((t) => {
...
function renderNode(n) {
    ...
    return (`<g data-node="${escapeXml(n.id)}" data-kind="${n.kind}" style="cursor:pointer">` +
        shell +
...
export function graphToSvg(graph, options = {}) {
    ...
    const orthogonalPaths = mode === "orthogonal" ? routeOrthogonalEdges(routedInputs) : null;
    const lanes = mode === "orthogonal" ? assignOrthogonalLanes(routedInputs) : new Map();
    const labeled = graph.edges
        ...
        .filter(Boolean);
    const resolvedLabels = resolveLabelOverlaps(labeled.map((l) => ({ id: l.id, x: l.x, y: l.y, w: l.w, h: l.h })), { gap: 10 });
    const edges = graph.edges
        ...
        if (e.label) {
            ...
            const textW = Math.max(estimateTextWidth(e.label, 10), e.contract ? estimateTextWidth(e.contract, 9) : 0);
        ...
        return (`<g data-edge="${escapeXml(e.id)}" data-kind="${escapeXml(e.kind)}" style="cursor:pointer">` +
