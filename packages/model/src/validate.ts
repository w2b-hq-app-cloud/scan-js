import type { ScanModel, SpherePort } from "./schema.js";
...
function issue(
  ...
  message: string,
  path: string,
  severity: ScanValidationSeverity = "error",
): ScanValidationIssue {
  return { code, message, path, severity };
}

function collectElements(model: ScanModel): ElementRef[] {
  ...
  for (const entry of entries) {
    ...
    if (prev) {
      issues.push(
        issue(
          code,
          `Duplicate id "${entry.id}" (also at ${prev})`,
          `${entry.path}.id`,
        ),
      );
    } else {
  ...
  for (let i = 0; i < ports.length; i++) {
    ...
    if (prev) {
      issues.push(
        issue(
          "duplicate-port-id",
          `Duplicate port id "${p.id}" on the same element (also at ${prev})`,
          path,
        ),
      );
    } else {
...
export function validateScanModel(model: ScanModel): ScanValidationResult {
  ...
  checkUniqueIds(
    elements.map((e) => ({ id: e.id, path: e.path })),
  ...
  for (const r of runtimeEntries) {
    if (elementById.has(r.id)) {
      issues.push(
        issue(
          "duplicate-element-id",
          `agent_runtime id "${r.id}" collides with an element id`,
          `${r.path}.id`,
        ),
      );
    }
    ...
    for (let i = 0; i < el.exposes.length; i++) {
      ...
      if (consumeIds.has(p.id)) {
        issues.push(
          issue(
            "duplicate-port-id",
            `Port id "${p.id}" used in both consumes and exposes on "${el.id}"`,
            `${el.path}.exposes[${i}].id`,
          ),
        );
      }
    }
    ...
    if (agent.runtime && !runtimeIds.has(agent.runtime)) {
      issues.push(
        issue(
          ...
          `Agent "${agent.id}" references unknown runtime "${agent.runtime}"`,
          `agents[${i}].runtime`,
        ),
      );
    }
  ...
  const connectionIdEntries: Array<{ id: string; path: string }> = [];
  for (let i = 0; i < model.connections.length; i++) {
    ...
    if (c.id) {
      ...
      if (elementById.has(c.id) || runtimeIds.has(c.id)) {
        issues.push(
          issue(
            "duplicate-element-id",
            `Connection id "${c.id}" collides with an element or runtime id`,
            `${base}.id`,
          ),
        );
      }
    ...
    if (!elementById.has(c.from)) {
      issues.push(
        issue(
          ...
          `Connection from "${c.from}" does not exist`,
          `${base}.from`,
        ),
      );
    }
    if (!elementById.has(c.to)) {
      issues.push(
        issue(
          "unknown-connection-endpoint",
          `Connection to "${c.to}" does not exist`,
          `${base}.to`,
        ),
      );
    }
    ...
    if (c.fromPort && fromEl) {
      if (!fromEl.exposes.some((p) => p.id === c.fromPort)) {
        issues.push(
          issue(
            "unknown-from-port",
            `fromPort "${c.fromPort}" is not an expose port on "${c.from}"`,
            `${base}.fromPort`,
          ),
        );
      }
    ...
    if (c.toPort && toEl) {
      if (!toEl.consumes.some((p) => p.id === c.toPort)) {
        issues.push(
          issue(
            "unknown-to-port",
            `toPort "${c.toPort}" is not a consume port on "${c.to}"`,
            `${base}.toPort`,
          ),
        );
      }
    ...
    for (let bi = 0; bi < view.boundaries.length; bi++) {
      ...
      for (let mi = 0; mi < b.members.length; mi++) {
        ...
        if (!elementById.has(member)) {
          issues.push(
            issue(
              "unknown-boundary-member",
              `Boundary "${b.id}" member "${member}" does not exist`,
              `views[${vi}].boundaries[${bi}].members[${mi}]`,
            ),
          );
        }
      }
    ...
    for (const layoutId of Object.keys(view.layout)) {
      if (!elementById.has(layoutId)) {
        issues.push(
          issue(
            "unknown-layout-id",
            `Layout key "${layoutId}" does not match any element`,
            `views[${vi}].layout.${layoutId}`,
          ),
        );
      }
    }
