...
export class Modeling {
   ...
   */
  changeElementKind(id: string, kind: CreateKind) {
    const prev = cloneModel(this.getModel());
    const next = cloneModel(prev);
    const located = locateElement(next, id);
    if (!located) throw new Error(`Element not found: ${id}`);
    if (located.kind === kind) return;
...
type ElementBag = "components" | "channels" | "external_systems" | "agents" | "repositories";
...
function locateElement(model: SphereModel, id: string): LocatedElement | null {
  const ci = model.components.findIndex((c) => c.id === id);
  if (ci >= 0) {
    const t = model.components[ci]!.type;
    const kind: CreateKind =
      t === "datastore" ? "datastore" : t === "search" ? "search" : "service";
    return { bag: "components", index: ci, kind };
  }
  const chi = model.channels.findIndex((c) => c.id === id);
  if (chi >= 0) return { bag: "channels", index: chi, kind: "event-stream" };
  const ei = model.external_systems.findIndex((c) => c.id === id);
  if (ei >= 0) return { bag: "external_systems", index: ei, kind: "external-system" };
  const ai = model.agents.findIndex((c) => c.id === id);
  if (ai >= 0) return { bag: "agents", index: ai, kind: "agent" };
  const ri = model.repositories.findIndex((c) => c.id === id);
  if (ri >= 0) return { bag: "repositories", index: ri, kind: "repository" };
  return null;
}
...
type ExtractedElement = {
  ...
  warn?: string;
  url?: string;
  repository?: string | { provider?: string; path: string };
  links?: Array<{ kind: "doc" | "repo" | "openapi" | "other"; href: string; title?: string }>;
  consumes?: Array<{ id: string; label: string; protocol?: string }>;
  exposes?: Array<{ id: string; label: string; protocol?: string }>;
  provider?: string;
  path?: string;
  purpose?: string;
};

function extractElement(model: SphereModel, located: LocatedElement): ExtractedElement {
  switch (located.bag) {
    case "components": {
...
function insertElementAsKind(model: SphereModel, kind: CreateKind, src: ExtractedElement) {
  const ports = {
