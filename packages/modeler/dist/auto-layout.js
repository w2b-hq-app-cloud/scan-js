...
function fanSides(base, index) {
    if (index === 0)
        return base;
    const horiz = base.fromSide === "l" || base.fromSide === "r";
    const fromCycle = horiz
        ? [base.fromSide, "t", "b"]
        : [base.fromSide, "l", "r"];
    const toCycle = horiz
        ? [base.toSide, "b", "t"]
 ...
 */
function splitTallLayers(byLayer, sizes, childrenOf, memberSet, gapY, maxH) {
    const out = [];
...
function assignClusters(ids, boundaries) {
    const ranked = [...boundaries].sort((a, b) => (a.members?.length ?? 0) - (b.members?.length ?? 0));
    const out = new Map();
    for (const id of ids) {
 ...
 */
function assignLayers(nodeIds, edges, model) {
    ...
    for (const e of edges) {
        if (!set.has(e.from) || !set.has(e.to) || e.from === e.to)
            continue;
...
function orderLayer(layerIds, prevOrder, edges, model) {
    ...
    const score = (id) => {
        ...
        if (!neighbors.length)
            return kindRank(model, id) * 1000;
        return neighbors.reduce((a, b) => a + b, 0) / neighbors.length;
    };
    return [...layerIds].sort((a, b) => score(a) - score(b) || kindRank(model, a) - kindRank(model, b) || a.localeCompare(b));
}
function layoutClusterMembers(memberIds, model, sizes, connections, gapX, gapY) {
    if (!memberIds.length) {
        return { local: {}, width: 0, height: 0 };
    }
    const memberSet = new Set(memberIds);
    const internal = connections.filter((c) => memberSet.has(c.from) && memberSet.has(c.to));
    // Attachments sit under their producer (DB / published events).
    const parentOf = new Map();
    const childrenOf = new Map();
    for (const c of internal) {
        if (!ATTACH_TYPES.has(c.type))
            continue;
        if (parentOf.has(c.to))
            continue;
    ...
    byLayer.splice(0, byLayer.length, ...splitTallLayers(byLayer, sizes, childrenOf, memberSet, gapY, MAX_LAYER_STACK_H));
    ...
    for (let li = 0; li < byLayer.length; li++) {
        ...
        for (const id of byLayer[li]) {
            ...
            if (kids.length && !pocketKids) {
                ...
                const rowW = kidWidths.reduce((a, b) => a + b, 0) + kidGap * Math.max(0, kids.length - 1);
 ...
 */
export function computeAutoLayout(model, viewId, options = {}) {
    ...
    if (freeIds.length) {
        ...
        const rest = freeIds.filter((id) => resolveEntityKind(model, id) !== "external" && resolveEntityKind(model, id) !== "repo");
        const pushFree = (cid, members) => {
            if (!members.length)
                return;
            const placed = layoutClusterMembers(members, model, sizes, model.connections, opts.gapX, opts.gapY);
            clusters.push({
