...
function buildIdMap(target, incoming) {
    const targetIds = new Set();
    for (const c of target.components)
        targetIds.add(c.id);
    for (const c of target.channels)
        targetIds.add(c.id);
    for (const c of target.external_systems)
        targetIds.add(c.id);
    for (const a of target.agents)
        targetIds.add(a.id);
    for (const r of target.agent_runtimes)
        targetIds.add(r.id);
    for (const r of target.repositories)
        targetIds.add(r.id);
    ...
    const kindPrefix = (id) => {
        if (incoming.components.some((c) => c.id === id))
            return "service";
        if (incoming.channels.some((c) => c.id === id))
            return "event-stream";
        if (incoming.external_systems.some((c) => c.id === id))
            return "external-system";
        if (incoming.agents.some((c) => c.id === id))
            return "agent";
        if (incoming.agent_runtimes.some((c) => c.id === id))
            return "runtime";
        if (incoming.repositories.some((c) => c.id === id))
            return "repository";
        return "boundary";
    };
    const map = new Map();
    ...
    for (const c of incoming.components)
        allIncomingIds.add(c.id);
    for (const c of incoming.channels)
        allIncomingIds.add(c.id);
    for (const c of incoming.external_systems)
        allIncomingIds.add(c.id);
    for (const a of incoming.agents)
        allIncomingIds.add(a.id);
    for (const r of incoming.agent_runtimes)
        allIncomingIds.add(r.id);
    for (const r of incoming.repositories)
        allIncomingIds.add(r.id);
    for (const view of incoming.views) {
        for (const b of view.boundaries)
            allIncomingIds.add(b.id);
    }
    ...
    const collectPorts = (arr) => {
        for (const el of arr) {
            for (const p of el.consumes ?? [])
                portIds.add(p.id);
            for (const p of el.exposes ?? [])
                portIds.add(p.id);
        }
    };
    ...
    const collectTargetPorts = (arr) => {
        for (const el of arr) {
            for (const p of el.consumes ?? [])
                targetPortIds.add(p.id);
            for (const p of el.exposes ?? [])
                targetPortIds.add(p.id);
        }
    };
    ...
    for (const id of portIds) {
        if (targetPortIds.has(id))
            map.set(id, createId("port"));
    }
    for (const conn of incoming.connections) {
        if (conn.id && targetIds.has(conn.id))
            map.set(conn.id, createId("conn"));
    }
    return map;
}
function mapped(map, id) {
...
function remapElement(el, map) {
    ...
    if (copy.consumes)
        copy.consumes = copy.consumes.map((p) => ({ ...p, id: mapped(map, p.id) }));
    if (copy.exposes)
        copy.exposes = copy.exposes.map((p) => ({ ...p, id: mapped(map, p.id) }));
    return copy;
}
/**
 ...
 */
export function mergeModels(target, incoming, viewId, options) {
    ...
    next.agents.push(...incoming.agents.map((a) => {
        const copy = remapElement(a, map);
        if (copy.runtime)
            copy.runtime = mapped(map, copy.runtime);
        return copy;
    }));
    next.agent_runtimes.push(...incoming.agent_runtimes.map((r) => ({ ...structuredClone(r), id: mapped(map, r.id) })));
    next.repositories.push(...incoming.repositories.map((c) => remapElement(c, map)));
    next.connections.push(...incoming.connections.map((conn) => ({
    ...
    if (!targetView || !incomingView || !nextView)
        return next;
