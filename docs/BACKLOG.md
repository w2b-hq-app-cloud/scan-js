# SCAN toolkit (`scan-js`) â€” Agent Backlog
...
1. **Notation** â€” schema, spec, validation, round-trip YAML (`Track A`, related `B`)
2. **Renderer** â€” faithful canvas + embed/export (`Track C`, whiteboard display) â€” must stay embeddable standalone
3. **Modeler** â€” create / connect / edit / undo that write the real model (`Track D`, `E` modeler UX)
4. **OSS polish & release** â€” CLI, docs, examples, publish `@spherescan/*` (`Track F`, `G`)
5. **Sphere AI / agents last** â€” NLâ†’model, real AI command bar, agent execution (`Track I` only).  
   Keep mocked Sphere chrome in the whiteboard; **implement** real AI later â€” do **not** remove chrome as cleanup.

---

## How to use with multiple agents

1. Pick an item with status `todo` (not `done` / `in_progress` / `deferred` / `cancelled`) in a track that does not overlap another agentâ€™s files.
2. Mark it `in_progress` when starting.
3. Keep PRs small; note acceptance criteria in the PR description.
4. **When finished, always set that itemâ€™s status to `done` in this file** (and add a one-line note under â€œRecently completedâ€ if useful). Do this in the same change set as the implementation â€” so the next agent sees an accurate board.

**Conflict hotspots (serialize agents):**

- [`apps/whiteboard/src/components/sphere/SphereApp.tsx`](apps/whiteboard/src/components/sphere/SphereApp.tsx) â€” only one agent at a time
- [`packages/model/src/schema.ts`](packages/model/src/schema.ts) â€” schema changes need coordinated tests
- Package `exports` / root `package.json` workspaces â€” coordinate if renaming packages

**Safe parallel tracks:** Model/spec, Rules, Viewer embed polish, CLI, Docs/OSS release (avoid two agents in `SphereApp.tsx`).

**Focus now:** SCAN notation + `@spherescan/viewer` + `@spherescan/modeler` (standalone-shippable). Sphere chrome in `apps/whiteboard` is a product shell around SCAN â€” leave mocked AI/collab UI in place until Track I.

---

## Recently completed (IDs â€” do not re-implement)
...
| **E3** | done | Port click targets for connect (expose â†’ consume) |
| **B1** | done | Port-aware rules (exposeâ†’consume when ports set) |
...
| **E5** | done | Context menu Duplicate + Ctrl/âŒ˜D |
| **A4** | done | `validateScanModel` â€” unique ids + connection/boundary/layout/runtime/port refs; CLI wired |
...
| Prefer | IDs | Notes |
|--------|-----|--------|
| Notation | **A2** | Unknown-key round-trip policy â€” A1/A4/A5 done |
| Rules | **B4**, **B2**, **B3** | Matrix / containment / soft warnings |
| UX polish | **E10** | Keyboard shortcuts (F2 rename, tools) â€” âŒ˜D duplicate done via E5 |
...
| Whiteboard | SCAN modeler UI wired to packages; YAML drop; connect feedback; New board; edge label/contract edit; Duplicate (menu + âŒ˜D) |
| OSS hygiene | Apache-2.0, CONTRIBUTING, CI, README, self-host doc |

---

## Track A â€” Notation & `@spherescan/model`
...
## Track B â€” `@spherescan/rules`

| ID | Status | Item | Acceptance criteria | Primary paths |
|----|--------|------|---------------------|---------------|
| B1 | done | Port-aware rules (exposeâ†’consume when ports exist) | Illegal port pair rejected with reason | `packages/rules/src/` |
| B2 | todo | Containment rules (who may live in trust vs runtime boundary) | Tests for member assignment | `packages/rules/src/` |
| B3 | todo | â€œMissing contractâ€ warnings as soft rules | Returns warnings without blocking connect | `packages/rules/src/` + whiteboard toast wiring |
...
## Track C â€” `@spherescan/viewer` (embed + export)
...
| D5 | todo | Drag performance: donâ€™t clone full model every pointer move | Preview path is cheap; commit still one command | `packages/modeler/src/modeling.ts`, `useSphereBoard.ts` |
| D6 | todo | Multi-select move command | N nodes one undo step | `packages/modeler/` |

---

## Track E â€” SCAN whiteboard UX (`apps/whiteboard`)

> Prefer agents that only touch `hooks/` or small components â€” avoid two agents in `packages/board`.  
...
| E3 | done | Port-level click targets for connect | Click expose â†’ consume wires ports | `@spherescan/board` + modeler D1 |
| E4 | done | Edge inspector: edit label/contract | Saves via modeler command | `@spherescan/board` + D3 |
| E5 | done | Wire context menu Duplicate | Uses D2; Ctrl/âŒ˜D | `@spherescan/board` ContextMenu |
| E6 | done | Empty board / New board action | Starts from A3 factory | Toolbar + modeler import |
| E7 | todo | Split `BoardApp.tsx` into modules (canvas, topbar, inspector, tools) | Files &lt; ~400 lines each; behavior unchanged | `packages/board/src/` â€” **solo agent** |
...
| F2 | todo | `scan export png` via optional dependency or document SVGâ†’PNG path | Documented; or puppeteer/sharp optional | `packages/cli/`, docs |
...
## Track G â€” Examples, docs, OSS release
...
| G3 | todo | More `examples/architectures/*.yaml` (2â€“3 boards) | Validate via CLI | `examples/architectures/` |
...
## Track H â€” Post-v0.1 enhancements (later)

| ID | Status | Item | Notes |
|----|--------|------|-------|
| H1 | done | Auto-layout (layered) as a modeler command | `modeling.autoLayout()` â€” no ELK dep yet |
...
| H5 | deferred | Collaborative editing / CRDT | Sphere product â€” after OSS modeler is solid |

---

## Track I â€” Sphere AI / agents (LAST â€” after stable SCAN notation + renderer + modeler)

> Do not start these until Tracks Aâ€“E core SCAN modeler paths are stable.  
> Keep existing mocked Sphere chrome in the whiteboard; **wire it** here â€” do not delete it (E11 cancelled).

| ID | Status | Item | Notes |
|----|--------|------|-------|
| I1 | deferred | NL â†’ SCAN model (prompt â†’ YAML/model patch) | Sphere product agents |
...
Formerly tracked as H6 (`NL â†’ model`). Explicitly last in priority order.

---

## Suggested multi-agent waves

### Wave 1 (parallel, low conflict) â€” notation / rules / export / CLI / docs

- Agent 1: ~~**A4**~~ **done** (id/ref validation) â€” A1+A5 done
- Agent 2: **A2** (unknown-key round-trip policy)
- Agent 3: **B1 + B4** (rules)
- Agent 4: **C2** (transparent PNG + scale) â€” C1 done
- Agent 5: **F1 + F3** (CLI)
- Agent 6: **G1 + G2** (docs/examples)

### Wave 2 â€” stable SCAN modeler

- Agent 1: ~~**D3 + E4**~~ **done**
- Agent 2: **D1** then **E3** (ports)
- Agent 3: ~~**D2 + E5**~~ **done** (duplicate)
- ~~**A3 + E6**~~ **done** Â· ~~**D4**~~ **done** (boundary resize)
- Also free: **E9**, **E10** â€” do **not** pick cancelled **E11**
...
- **G4 + G5 + G7** + version bump + publish (human-gated) â€” publishable `@spherescan/*` packages

### Wave 5 (last) â€” Sphere AI / agents

- **Track I** â€” wire mocked Sphere chrome to real agents after SCAN renderer/modeler are solid
...
- [ ] **Backlog row status â†’ `done`** (required before handoff)
- [ ] Update â€œRecently completedâ€ / â€œSuggested next pickupsâ€ if the next unblocked set changed
